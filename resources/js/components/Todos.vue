                ...
                <div class="card" style="width: 18rem;" v-if="todo.done == 0">
                    <div class="card-body">
                        <h5 class="card-title">{{todo.content}}</h5>
                         <div class="mb-2">
                            タグ：<span v-for="(key, index) in todo.tags" :key="index"><small class="mr-1">{{todo.tags[index].title}}</small></span>
                        </div>
                    ...
                    <div class="btn-group position-absolute dropdown" style="top:10px; right:4px;">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <div class="text-center text-success" v-on:click="doneTodoUpdate(todo)" v-if="todo.done == 0">完了</div>
                            <div class="text-center" v-on:click="doneTodoUpdate(todo)" v-if="todo.done == 1">未完了</div>
                            <div class="text-center" data-toggle="modal" :data-target="'#todoModal'+goalId+todo.id" v-on:click="content = todo.content">編集</div>
                            <div class="text-center" data-toggle="modal" :data-target="'#todoSortModal'+goalId+todo.id" v-on:click="content = todo.content">並びかえ</div>
                            <div class="text-center text-danger" v-on:click="deleteTodo(todo)">削除</div>
                        </div>
                    </div>
                </div>
